...
const AddProduct = () => {
  ...
  const removeFlavourRow = (category, index) => {
    setFormData((prev) => {
      ...
      return {
        ...prev,
        [category]: updated.length ? updated : [{ ...emptyRow }],
      };
    });
  };
  ...
  const handleSubmit = async (e) => {
    ...
    data.append(
      "featuresPrice",
      JSON.stringify({ eggless: 100, shape: 100, fondant: 600 })
    );
      ...
      {formData[category].map((f, i) => (
        <div key={i} className="ap-flavour-row">
          <input
            ...
            value={f.name}
            onChange={(e) =>
              handleFlavourChange(category, i, "name", e.target.value)
            }
          />
          <input
            type="number"
            ...
            value={f.price}
            onChange={(e) =>
              handleFlavourChange(category, i, "price", e.target.value)
            }
          />
          <button
            type="button"
  ...
  return (
    <div className="addproduct-wrap">
      ...
      >
        ...
        <hr className="ap-divider" />
        {renderFlavourRows("exotic", "Exotic")}
